...
C:字面值常量的分类：字符串常量、整数常量、小数常量、字符常量(**一个字符**)、布尔常量、空常量(`null`)
...
计算机都是以补码存储和计算的，以下示例可以说明采用补码的原因
...
byte b4 = 3 + 4;    // 正常，Java有常量优化机制，只要数值再类型范围内就不会报错
```

报错原因分析：
  * byte 类型的变量在进行运算的时候，会自动类型提升为 int 类型
  * b1 和 b2 是两个变量，变量里面存储的值都是变化的，所以编译时无法判断里面具体的值
